...
from .button_style import ButtonStyle, _RESET_COLOR
...
class Component(MatrixButtons, LinearButtons, ColumnButtons):
    ...
    def __init__(self, buttons: list[Button],
                        global_buttons_style: ButtonStyle = ButtonStyle(**DEFAULT_STYLE), # type: ignore
        ...
        for button in buttons:
            if button.key_bind_keys is not None:
                if button._key_bind and self._display_index:
                    raise KeyBindError("Key bind and display index can't be use at the same time.")
                self._binded_keys += button.key_bind_keys # type: ignore
        if len(self._binded_keys) != len(set(self._binded_keys)):
            raise KeyBindError("Duplicate key accross two differents buttons.")
        self._size = 0
    ...
    def _build_button_style(self, label: str,
        ...
        return (
            ...
            f"{style.text_color if selected else ''}"
            f"{style.highlight_color if selected else ''}"
